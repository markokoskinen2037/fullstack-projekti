...
goalsRouter.post('/', async (request, response) => {
  ...
  const result = user.goals.find(
    goal => goal.course.equals(newGoal.course) && goal.user.equals(newGoal.user)
  )
  ...
  if (result !== undefined) {
    ...
    return response
      .status(204)
      .json({ error: 'this user-course combination already exists!' })
  }
  ...
  newGoal.save().then(savedGoal => {
    ...
    User.findByIdAndUpdate(request.body.userid, user).then(updatedUser => {
...
goalsRouter.delete('/:id', (request, response) => {
  ...
  Goal.findByIdAndRemove(request.params.id)
    .then(() => {
...
goalsRouter.put('/:id', (request, response) => {
  ...
  Goal.findByIdAndUpdate(request.params.id, updatedGoal, {
    new: true,
  })
    .then(updatedCourse => {
